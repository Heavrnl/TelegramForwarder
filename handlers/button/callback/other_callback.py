...
async def callback_copy_rule(event, rule_id, session, message, data):
    ...
    After selection, current rule settings will be copied to the target rule.
    """
    try:
        ...
        await event.edit("Please select the target rule to copy the current rule to:", buttons=buttons)
    except Exception as e:
        logger.error(f"Error showing copy rule selection interface: {str(e)}")
...
async def create_copy_rule_buttons(rule_id, page=0):
    ...
    try:
        ...
        for rule in current_page_rules:
            ...
            # Create callback data: perform_copy_rule:source_rule_id:target_rule_id
...
async def callback_perform_copy_rule(event, rule_id_data, session, message, data):
    ...
    try:
        ...
        if len(parts) != 2:
            await event.answer("Data format error")
            return

        source_rule_id = int(parts[0])
        target_rule_id = int(parts[1])

        # Get source rule and target rule
        ...
        if source_rule.id == target_rule.id:
            await event.answer('Cannot copy rule to itself')
            return
        ...
        for keyword in source_rule.keywords:
            if not keyword.is_regex:  # Plain keywords
        ...
        # Copy regex keywords
        for keyword in source_rule.keywords:
            if keyword.is_regex:  # Regex keywords
                # Check if already exists
        ...
        # Copy rule settings
        # Save target rule's original associations
        ...
        # Restore target rule's original associations
...
async def callback_copy_keyword(event, rule_id, session, message, data):
    ...
    Shows a list of selectable rules for the user to choose the target rule to copy keywords to.
    After selection, keywords from the current rule will be copied to the target rule.
    """
    try:
        # Call the generic rule selection function
        await show_rule_selection(
            event, rule_id, data, "Please select the target rule to copy keywords from the current rule to:", "perform_copy_keyword"
        )
    except Exception as e:
...
async def callback_copy_replace(event, rule_id, session, message, data):
    ...
    Shows a list of selectable rules for the user to choose the target rule to copy replace rules to.
...
async def callback_perform_copy_keyword(event, rule_id_data, session, message, data):
    """Execute copy keyword operation
...
async def create_rule_selection_buttons(rule_id, page=0, callback_action="perform_copy_rule"):
    ...
    Args:
        rule_id: Current rule ID
        page: Current page number
        callback_action: Callback action after button click
    ...
    try:
        ...
        for rule in current_page_rules:
            ...
            # Create callback data: callback_action:source_rule_id:target_rule_id
...
async def callback_perform_clear_keyword(event, rule_id_data, session, message, data):
    ...
    try:
        ...
        await event.answer(f"Cleared all keywords for rule {rule.id}")
...
async def callback_perform_clear_replace(event, rule_id_data, session, message, data):
    ...
    try:
        ...
        await event.answer(f"Cleared all replace rules for rule {rule.id}")
...
async def callback_perform_delete_rule(event, rule_id_data, session, message, data):
    ...
    try:
        ...
        # Use generic method to check and clean up unused chat records
...
async def callback_set_userinfo_template(event, rule_id, session, message, data):
    ...
    logger.info(f"Starting to process set user info template callback - event: {event}, rule_id: {rule_id}")
...
async def callback_set_time_template(event, rule_id, session, message, data):
    """Set time template"""
    logger.info(f"Starting to process set time template callback - event: {event}, rule_id: {rule_id}")
...
async def callback_set_original_link_template(event, rule_id, session, message, data):
    ...
    logger.info(f"Starting to process set original link template callback - event: {event}, rule_id: {rule_id}")
...
async def callback_cancel_set_original_link(event, rule_id, session, message, data):
    """Cancel set original link template"""
    rule_id = data.split(':')[1]
    try:
        rule = session.query(ForwardRule).get(int(rule_id))
        if rule:
