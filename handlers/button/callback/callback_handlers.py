...
async def callback_page_rule(event, page_str, session, message, data):
    ...
    try:
        ...
        await event.answer('Error handling pagination request, please check logs')
...
async def update_rule_setting(event, rule_id, session, message, field_name, config, setting_type):
    ...
    try:
        # First update current rule
        session.commit()
        logger.info(f'Updated rule {rule.id} {field_name} from {current_value} to {new_value}')

        # Check if sync is enabled, and it's not the "enable_rule" or "enable_sync" field
        if rule.enable_sync and field_name != 'enable_rule' and field_name != 'enable_sync':
            logger.info(f"Rule {rule.id} has sync enabled, syncing setting changes to associated rules")
            ...
            for sync_rule in sync_rules:
                ...
                try:
                    ...
                    logger.error(f"Error syncing settings to rule {sync_rule_id}: {str(e)}")
                    continue
        ...
        await event.answer('Failed to update setting, please check logs')
        return False
...
async def handle_callback(event):
    ...
    try:
        ...
        try:  
            ...
            if handler:
                logger.info(f'Found corresponding handler: {handler}')
                await handler(event, rule_id, session, message, data)
            else:
                logger.info(f'No corresponding handler found, trying rule setting toggle: {action}')
