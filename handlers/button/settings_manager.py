...
RULE_SETTINGS = {
    ...
    'is_filter_user_info': {
        'display_name': 'Include sender name and ID when filtering keywords',
        'values': {
            True: 'Yes',
            False: 'No'
        },
    ...
    'use_bot': {
        'display_name': 'Forward method',
        'values': {
            True: 'Use bot',
            False: 'Use user account'
        },
    ...
    'is_preview': {
        ...
        'values': {
            PreviewMode.ON: 'On',
            PreviewMode.OFF: 'Off',
            PreviewMode.FOLLOW: 'Follow original message'
        },
...
AI_SETTINGS = {
    ...
    'is_keyword_after_ai': {
        'display_name': 'Run keyword filter again after AI processing',
        'values': {
            True: 'On',
            False: 'Off'
        },
    ...
    'summary_now': {
        'display_name': 'Execute summary now',
...
MEDIA_SETTINGS = {
    ...
    'is_send_over_media_size_message': {
        'display_name': 'Send alert when media size exceeds limit',
        'values': {
            True: 'On',
            False: 'Off'
        },
...
async def create_buttons(rule):
    ...
    # Get the currently selected rule for the current chat
    session = get_session()
    try:
        ...
        # Whether to filter user info
        buttons.append([
            Button.inline(
                f"Include sender name and ID when filtering: {RULE_SETTINGS['is_filter_user_info']['values'][rule.is_filter_user_info]}",
        ...
        buttons.append([
            ...
            Button.inline(
                f"🤖 Forward method: {RULE_SETTINGS['use_bot']['values'][rule.use_bot]}",
                f"toggle_bot:{rule.id}"
            )
        ])


        if rule.use_bot:  # Only show these settings when using bot
            buttons.append([
                Button.inline(
            ...
            buttons.append([
                Button.inline(
                    f"👁 Preview mode: {RULE_SETTINGS['is_preview']['values'][rule.is_preview]}",
            ...
            buttons.append([
                ...
                Button.inline(
                    f"⌛ Delay: {rule.delay_seconds or 5}s",
